...
{
	cout << "Ricevuto segnale #" << signal << endl;
    exit_now = true;
}
...
void on_keypress(void* not_used, const CEC::cec_keypress *msg)
{
	...
	std::cout << "on_keypress [" << msg->duration << "]: " << static_cast<int>(msg->keycode) << " " << key << std::endl;
	return;
}
...
{
	cout << ProgramName << " starting..." << endl;
...
//	int8_t devices_found = cec_adapter->FindAdapters(devices, 10, NULL); // TODO: deprecated, use DetectAdapters instead
	if( devices_found <= 0)
	{
	...
	{
		cout << "trovati " << (int)devices_found << " dispositivi" << endl;
		cout << " comm: " << devices[0].strComName << endl;
		cout << " path: " << devices[0].strComPath << endl;
	}
...
# if 1
	...
	cout << "Ready." << endl;
	int cicli = 0;
	//while( !exit_now ) // without while -> segmentation fault
	while (cicli++ < 30)
	{
